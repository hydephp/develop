...
HydePHP v3 removes the `InMemoryPage` instance macro API rather than deprecating it. Since v3 is a major release,
lazy closure contents directly replace the common dynamic-content use case without carrying a transitional API.
...
Arbitrary instance macros are no longer supported. If you previously registered a macro other than `compile`, or need
complete control over compilation, create a custom page class that extends `InMemoryPage` and override the relevant method.
...
- [ ] Moved `InMemoryPage` `compile` macro callbacks into the contents argument and replaced other macros with a custom page subclass
