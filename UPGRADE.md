...
## Step 5: Migrate `InMemoryPage` `compile` Macros

HydePHP v3 no longer allows an `InMemoryPage` macro named `compile` to override the real `compile()` method. Since v3
is a major release, lazy closure contents directly replace this dynamic-content special case without a transitional path.
...
- [ ] Moved `InMemoryPage` `compile` macro callbacks into the contents argument
