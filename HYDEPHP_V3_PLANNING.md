...
- Pages with non-HTML output paths are excluded from automatic navigation by default. Set `navigation.visible: true` or `navigation.hidden: false` to opt a non-HTML page into the generated navigation.
...
- Add `navigation.visible: true` or `navigation.hidden: false` to non-HTML pages that should remain in automatic navigation.
