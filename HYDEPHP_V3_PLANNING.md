...
- Blog posts dated in the future are no longer built, though they remain visible when serving the site. Run a build and review the warnings to find posts that are now skipped, correcting any mistyped dates. If you want to schedule posts, set up recurring builds, as a scheduled post is only published by a build run after its date has passed.
