...
## Terminal block style tag motivation

The tag vocabulary follows the console output formatter that Laravel and Hyde commands are written against. The tags
never appear in output a reader could copy, since the formatter turns them into escape codes before anything reaches
the terminal; they are written by hand to mark up output that was pasted as plain text. Following the console syntax
means that markup is the one the author already writes in their own commands, instead of a second vocabulary invented
for documentation. That covers the four named styles, the `fg`, `bg`, and `options` attributes separated by
semicolons, and the `</>` shorthand closing tag. The colors are the sixteen ANSI colors, taken from the Material
Palenight palette the terminal view already uses. That is where `gray` comes from: the muted gray that console output
leans on for secondary detail.

`strikethrough` is deliberately not one of the console formatter's options, which stop at `bold`, `underscore`,
`blink`, `reverse`, and `conceal`. It has an obvious rendering on a page and an obvious use in documentation, so it is
supported as an addition. The three console options with no sound rendering on a page are not supported at all, nor
are the `href` attribute and the `default` color, since each would amount to a tag that is recognized and then does
nothing. They stay literal text, as unknown colors, options, and attributes already do. A tag is only a style tag
when the whole of it is a semicolon-separated list of attribute pairs, so a stray word does not make the rest of a
malformed tag take effect.
...
Colors are written out as complete Tailwind class strings rather than composed from a hex value at runtime, since the
class names have to appear literally in the source for the stylesheet compiler to find them. Each span also carries a
`hyde-terminal-*` hook class next to the utility classes, matching the hooks the rest of the terminal markup provides.
