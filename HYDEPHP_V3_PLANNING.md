...
- Publishing is now driven by a single `publish` command that replaces the three `publish:*` commands from v2. Run it without options to pick between publishing views and a starter page, or skip straight to what you want with `--layouts`, `--components`, `--all`, or `--page[=NAME]`. Views go to `resources/views/vendor/hyde`, starter pages (`welcome`, `posts`, `blank`, and `404`) go to the Blade page source directory, and `--to` places a named page somewhere else. Publishing is safe by default: a destination that already matches its source is skipped, and one you have modified is left alone unless you confirm the overwrite or pass `--force`. The comparison ignores line endings, so a CRLF checkout does not read as a modification. Starter pages come from the new `PublishablePages` registry, which packages can add their own pages to. Raw package, provider, and tag publishing stays with `vendor:publish`, which the `publish` command deliberately does not expose.
...
- Removed the `publish:views`, `publish:homepage`, and `publish:configs` commands in favor of the unified `publish` command. They are removed outright rather than aliased, so invoking one produces Symfony's command-not-found error, which already suggests `publish`. The Hyde configuration files moved to a plain Laravel publish tag, `php hyde vendor:publish --tag=hyde-config`, which publishes `hyde.php`, `docs.php`, `markdown.php`, `view.php`, `cache.php`, and `commands.php`; the legacy `configs`, `hyde-configs`, and `support-configs` tags are removed, and Torchlight's config is published through Torchlight's own tag. Unlike the old commands, publishing no longer overwrites files you have modified without confirmation or `--force`.
...
- Replace `publish:views` with `publish --all` (or `--layouts` / `--components`), `publish:homepage` with `publish --page`, and `publish:configs` with `vendor:publish --tag=hyde-config --force`. The `publish:homepage posts` and `publish:homepage blank` replacements need `--to=_pages/index.blade.php` to keep publishing to the homepage.
