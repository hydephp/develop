...
[![Framework Tests (Matrix)](https://github.com/hydephp/framework/actions/workflows/run-tests.yml/badge.svg?branch=1.x)](https://github.com/hydephp/framework/actions/workflows/run-tests.yml)
[![Hyde Tests](https://github.com/hydephp/hyde/actions/workflows/run-tests.yml/badge.svg?branch=1.x)](https://github.com/hydephp/hyde/actions/workflows/run-tests.yml)
</div>
...
The release helper targets `1.x` by default on this branch. Pass another version branch explicitly when preparing a release from a different line. Once the monorepo release pull request is merged, the matching branch is synchronized to the Hyde and Framework repositories before their releases are published.
