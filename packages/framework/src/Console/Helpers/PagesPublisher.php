...
class PagesPublisher extends BasePublisher
{
    ...
    {
        if (in_array('..', $this->pathSegments($path), true)) {
            return false;
        }
        ...
        return $this->targetIsWithinSourceDirectory($normalized, $pageClass)
            && Str::endsWith($normalized, $pageClass::sourceExtension());
    }

    /** Normalize a project-relative path so that equivalent spellings of the same destination compare equal. */
    protected function normalizeTargetPath(string $path): string
    {
        return implode('/', $this->pathSegments($path));
    }
    ...
    protected function pathSegments(string $path): array
    {
        return array_values(array_filter(
            explode('/', normalize_slashes($path)),
    ...
    protected function targetIsWithinSourceDirectory(string $target, string $pageClass): bool
    {
        ...
        return $sourceDirectory === '' || Str::startsWith($target, "$sourceDirectory/");
    }
    ...
    protected function assertNoDestinationConflicts(array $resolved): bool
    {
        ...
        foreach ($labelsByTarget as $target => $labels) {
            if (count($labels) > 1) {
                ...
                $this->console->error(sprintf('%s %s %s.', Arr::join($labels, ', ', ' and '), $verb, $target));
