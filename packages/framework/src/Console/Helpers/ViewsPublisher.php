...
class ViewsPublisher extends BasePublisher
{
    ...
    public function publish(): int
    {
        ...
        $published = $this->recordsToMap(array_merge($copy, $overwrite));
    ...
    protected function decide(array $selected): array
    {
        ...
        foreach ($selected as $source => $target) {
            ...
            $record = ['source' => $source, 'target' => $target, 'absolute' => $target];
    ...
    protected function publishedLine(array $published, int $offeredTotal): string
    {
        ...
        return $count === $offeredTotal
            ? sprintf('Published all %d views to [%s]', $count, $this->baseDirectory($published))
            : sprintf('Published %d views to [%s]', $count, $this->baseDirectory($published));
    }
