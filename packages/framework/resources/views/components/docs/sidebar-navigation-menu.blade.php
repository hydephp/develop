<ul id="sidebar-navigation-menu" role="list">
	...
	<li @class(['sidebar-navigation-item -ml-4 pl-4' , 'active bg-darken'=> $item->destination === basename($currentPage)])>
		...
		<a href="{{ Hyde::pageLink($item->destination . '.html') }}" aria-current="true" class="-ml-4 p-2 block  text-indigo-600 border-l-[0.325rem] border-indigo-500 transition-colors duration-300	ease-in-out">{{
			$item->label }}</a>
