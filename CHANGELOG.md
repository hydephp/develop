...
- Added built-in `terminal` fenced code blocks with command prompt styling, optional window titles using the `terminal title="Build output"` info string, and optional Symfony Console formatting using the `terminal xml` info string. They render through the publishable terminal view, which is backed by the new `TerminalBlockComponent` Blade component that can also be constructed and rendered on its own. ([#2188](https://github.com/hydephp/develop/issues/2188), [#2485](https://github.com/hydephp/develop/issues/2485))
