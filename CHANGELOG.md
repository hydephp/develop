...
- Added built-in `terminal` fenced code blocks with command prompt styling, an optional window title using the `title="…"` modifier, and optional XML style formatting using the `terminal xml` info string, which styles the `<info>`, `<comment>`, `<question>`, and `<error>` tags, as well as colors and text formatting set with `fg`, `bg`, and `options` attributes. ([#2188](https://github.com/hydephp/develop/issues/2188), [#2485](https://github.com/hydephp/develop/issues/2485))
